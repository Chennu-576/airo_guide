...
export function Navigation({ onToggleSidebar }: NavigationProps) {
  return (
    <header className="bg-[#a836ba] text-white flex-shrink-0 sticky top-0 z-40">
      {/* Row 1: logo + login */}
      <div className="flex items-center justify-between px-10 lg:px-12 h-11 border-b border-white/20">
        <div className="flex items-center gap-2">
          <button
            className="lg:hidden p-1 mr-1 text-white/80 hover:text-white"
            onClick={onToggleSidebar}
      ...
      {/* Row 2: tabs + search */}
      <div className="flex items-center justify-between px-10 lg:px-12 h-10">
        ...
        <div className="flex items-center gap-2 bg-white rounded-md px-3 py-1.5 text-xs text-gray-400 shadow-sm cursor-pointer hover:shadow">
          <Search className="w-3.5 h-3.5" />
          <span>Search</span>
          <kbd className="ml-2 px-1.5 py-0.5 bg-gray-100 rounded text-[10px] text-gray-500 font-mono">
            CTRL+K
          </kbd>
        </div>
      </div>
